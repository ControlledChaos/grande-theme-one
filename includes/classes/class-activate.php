...
class Activate {
	...
	public function theme_switch() {
		...
		$needs_update = version_compare( phpversion(), GRANDE_PHP_VERSION, '<' );
	...
	?>
		<div id="wilbur-php-notice" class="notice notice-error is-dismissible">
			<?php echo sprintf(
				'<p>%1s %2s %3s %4s</p>',
				__( 'The Grande Design theme could not be activated because it requires PHP version', 'grande' ),
				GRANDE_PHP_VERSION,
				__( 'or greater. Your system is running PHP version', 'grande' ),
				phpversion()
			); ?>
		</div>
	<?php
